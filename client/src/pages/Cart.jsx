...
const Cart = () => {
    ...
    const getUserAddress = async () => {
        try {
            const { data } = await axios.get('/lps/address/get');
            if (data.success) {
    ...
    const placeOrder = async () => {
        try {
            ...
            if (paymentOption === 'COD') {
                const { data } = await axios.post("/lps/order/cod", {
                    userId: user._id,
            ...
            // place order with stripe
                ...
                const { data } = await axios.post("/lps/order/stripe", {
                    userId: user._id,
                    items: cartArray.map(item => ({
                        product: item._id,
                        quantity: item.quantity
    ...
    return products.length > 0 && cartItems ? (
        cartIsEmpty ? (
            <div className="mt-20 text-center text-gray-500 text-lg">
                Your cart is empty.
                <br />
                <button onClick={() => navigate("/products")} className="mt-4 text-primary hover:underline">
                    Go to products
                </button>
            </div>
        ) : (
            <div className="flex flex-col md:flex-row mt-16">
                ...
                <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                    ...
                    >
                        {paymentOption === "COD" ? "Place Order" : "Proceed to Online Checkout"}
                    </button>
                </div>
            </div>
        )
