    ...
    const logout=async()=>{
        ...
        // navigate("/")
         try {
                         ...
                         const {data}=await axios.get('/lps/user/logout')
                         if(data.success){
  ...
  return (
    ...
    <div className="hidden sm:flex items-center gap-8">
        ...
        {!user ? (<button onClick={()=>setShowUserLogin(true)} className="cursor-pointer px-8 py-2 bg-primary hover:bg-primary-dull transition text-white rounded-full">
            ...
            <ul className='hidden group-hover:block absolute top-10 rifgt-0 bg-white shadow border border-gray-200 py-2.5 w-30 rounded-md text-sm z-40'>
...
<div className='flext items-center gap-6 sm:hidden'>
